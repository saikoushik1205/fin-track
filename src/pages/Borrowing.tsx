...
export const Borrowing = () => {
  ...
  const lenders = getPeople("lender");
  const filteredLenders = lenders.filter((person) =>
    person.name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const selectedLender = lenders.find((p) => p.name === selectedPerson);

  const totalBorrowed = lenders.reduce((sum, p) => sum + p.totalAmount, 0);
  const totalReturned = lenders.reduce((sum, p) => sum + p.amountReturned, 0);
  const totalOwed = lenders.reduce((sum, p) => sum + p.remainingBalance, 0);
  ...
  const handleDeletePerson = (personName: string) => {
    const person = lenders.find((p) => p.name === personName);
    if (!person) return;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-dark-400 text-sm mb-1">Total Borrowed</p>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-dark-400 text-sm mb-1">Total Repaid</p>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-dark-400 text-sm mb-1">Amount Owed</p>
      ...
      {filteredLenders.length > 0 ? (
        ...
        >
          {filteredLenders.map((person, index) => (
            <motion.div
              key={person.name}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
        ...
        <EmptyState
          title={searchQuery ? "No lenders found" : "No borrowing records yet"}
          description={
            searchQuery
              ? "Try adjusting your search query"
              : "Start tracking by adding your first borrowing transaction"
          }
          actionLabel={!searchQuery ? "Add Borrowing" : undefined}
      ...
      <Modal
        isOpen={!!selectedPerson && !!selectedLender}
        onClose={() => setSelectedPerson(null)}
        title={selectedLender?.name || ""}
        size="lg"
      >
        {selectedLender && (
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                onClick={() =>
                  handleAddTransactionForPerson(selectedLender.name)
                }
              ...
              <ExportPDFButton
                personName={selectedLender.name}
                transactions={selectedLender.transactions}
                type="borrowed"
              />
              <button
                onClick={() => handleDeletePerson(selectedLender.name)}
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="glass-card p-4">
                ...
                <p className="text-2xl font-bold text-dark-50">
                  ₹{selectedLender.totalAmount.toLocaleString("en-IN")}
                </p>
              </div>
              <div className="glass-card p-4">
                <p className="text-dark-400 text-sm mb-1">Repaid</p>
                <p className="text-2xl font-bold text-green-400">
                  ₹{selectedLender.amountReturned.toLocaleString("en-IN")}
                </p>
              </div>
              <div className="glass-card p-4">
                <p className="text-dark-400 text-sm mb-1">Remaining</p>
                <p className="text-2xl font-bold text-red-400">
                  ₹{selectedLender.remainingBalance.toLocaleString("en-IN")}
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {selectedLender.transactions
                  .filter((t) => !t.parentId)
                  .map((transaction) => {
                    const childTransactions =
                      selectedLender.transactions.filter(
